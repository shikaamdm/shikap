...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Aguarde, tou fazendo garai⏳',
				success: '[ ✔ ]Funciona',
				...
				only: {
					...
					ownerG: '[❗] Tué o Dono do chat(Shika)?😂',
					ownerB: '[❗] Tué o Shika?😂',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo!',
					Badmin: '[❗] Sou adm n garai, qual foi shika?!',
                                        daftarB: `[ REGISTRE-SE ]\nOlá !\nVoce  ainda nao está registrado no banco de dados! \n\nComando : ${prefix}rg nome|idade\nExemplo : ${prefix}rg Shika|15`,
				}
			}
			...
			const ownerNumber = ["558191153393@s.whatsapp.net"] // Nomer Owner
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'Shika'//Nama Bot
                        const ownerName = 'Shika<3'//Nama Owner
               ...
               break
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, esse vagabundo(a) aqui @${mentioned[0].split('@')[0]}\n\nVirou membro comum no *${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui : @${mentioned[0].split('@')[0]} Agr é adimin,Então da uma sugada nele!😎🤙`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Shika", vcard: vcard}, MessageType.contact, { quoted: mek})
